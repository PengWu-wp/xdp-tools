...
#define PROG_NAME "nicache_user"

static const char *map_name = "cache_map";
static const char *file_path = "/sys/fs/bpf/cache_map";
static const char *file_name[1] = {"nicache_kern.o"};
...
static struct prog_option load_options[] = {
        ...
        .metavar = "<filenames>",
//        .required = true,
        .help = "Load programs from <filenames>, default nicache_kern.o"),
        END_OPTIONS
};
...
int do_load(const void *cfg, __unused const char *pin_root_path) {
    ...
    for (i = 0; i < num_progs; i++) {
        ...
        if (!opt->pin_path) {
            ...
            bpf_object__for_each_map(map, xdp_program__bpf_obj(p))
            {
                ...
                if (err) {
                    printf("Error clearing map pin path: %s\n",
                           strerror(-err));
    ...
    printf("Success: XDP program attached to '%s'\n", opt->iface.ifname);

    int map_fd = bpf_object__find_map_fd_by_name(xdp_program__bpf_obj(p), map_name);
//    printf("found map_fd is %d\n", map_fd);
...
int do_unload(const void *cfg, __unused const char *pin_root_path) {
    ...
    if (opt->all) {
        ...
        printf("Success: all XDP programs detached on iface %s\n", opt->iface.ifname);
    } else {
    ...
    if (err) {
        ...
        printf("Success: pinned map removed\n");
    }
...
static struct prog_option map_options[] = {
        ...
        DEFINE_OPTION("value", OPT_STRING, struct mapopt, value,
        .metavar = "[value]",
        .short_opt = 'V',
        .help = "Specify the value"),
        DEFINE_OPTION("delete", OPT_BOOL, struct mapopt, delete,
        .short_opt = 'D',
        .help = "Performing deletion"),
        END_OPTIONS
};
...
int do_map(const void *cfg, __unused const char *pin_root_path) {
    ...
    int err;
    struct key_entry key = {0};
    struct cache_entry value = {0};

    unsigned int key_len = strlen(opt->key);
    unsigned int value_len;
    if (!opt->delete && !opt->value) {
        pr_warn("Performing map update but no value provided. Add -V/--value\n");
        return EXIT_FAILURE;
    } else if (!opt->delete) {
        value_len = strlen(opt->value);
        memcpy(value.data, opt->value, value_len);
        value.len[0] = value_len % 10 + '0';
        value.len[1] = value_len / 10 + '0';
    }
    memcpy(key.data, opt->key, key_len);

    int map_fd = bpf_obj_get(file_path);
    if (map_fd < 0) {
    ...
    if (!opt->delete) {
        ...
        printf("Success: key '%s' deleted in map\n", key.data);
        return 0;
    }
    return 0;
}
